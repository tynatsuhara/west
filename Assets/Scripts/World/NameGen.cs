...
public class NameGen {
	...
	private static NameGen personNameGen = new NameGen(new string[] {
		"<male-name>:<male-first> [last]",
		"<female-name>:<female-first> [last]",
	...
	public static string CharacterName(bool female = false, string lastName = "") {
		if (lastName.Length == 0)
			lastName = personNameGen.Generate("<last>");
		string name = personNameGen.Generate(female ? "<female-name>" : "<male-name>");
		return name.Replace("[last]", lastName);
	}
}